"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.IndicadoresController = void 0;
const common_1 = require("@nestjs/common");
const indicadores_service_1 = require("./indicadores.service");
let IndicadoresController = class IndicadoresController {
    constructor(service) {
        this.service = service;
    }
    async indicadores(startDate, endDate, typeMaintenance) {
        const types = typeMaintenance
            ? typeMaintenance.split(',').map((t) => Number(t))
            : undefined;
        const data = await this.service.obterIndicadores({
            startDate,
            endDate,
            typeMaintenance: types,
        });
        return { success: true, data };
    }
};
exports.IndicadoresController = IndicadoresController;
__decorate([
    (0, common_1.Get)('performance-indicator'),
    __param(0, (0, common_1.Query)('startDate')),
    __param(1, (0, common_1.Query)('endDate')),
    __param(2, (0, common_1.Query)('typeMaintenance')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, String]),
    __metadata("design:returntype", Promise)
], IndicadoresController.prototype, "indicadores", null);
exports.IndicadoresController = IndicadoresController = __decorate([
    (0, common_1.Controller)('maintenance/reports'),
    __metadata("design:paramtypes", [indicadores_service_1.IndicadoresService])
], IndicadoresController);
//# sourceMappingURL=indicadores.controller.js.map